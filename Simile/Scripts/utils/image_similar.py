...
class HashSimilar(object):
    ...
    @staticmethod
    def calculateDifference(pathResultImage, pathReferenceImage) -> float:
        resultImage = cv2.imread(pathResultImage)
        cv2.imwrite('resultImage.png', resultImage)
        refeferenceImage = cv2.imread(pathReferenceImage)
        refeferenceImage = cv2.cvtColor(refeferenceImage, cv2.COLOR_BGR2GRAY)
        refeferenceImage = cv2.GaussianBlur(refeferenceImage, (5, 5), 1.0)
        cv2.imwrite('referenceImage.png', refeferenceImage)

        print(HashSimilar.get_attention_similar(
            'store' + os.path.sep + 'resultImage.png', 
            'referenceImage.png'))
