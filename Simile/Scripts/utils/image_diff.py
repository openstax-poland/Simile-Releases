...
class ImageDiff(object):
    def __init__(self, w=9, padding=0, w_scale=825, h_scale=0, hash_score=0.85, pixel_value=28):
        self.filter_w = w
    ...
    def increment_diff(self, image1, image2, image_show):
        ...
        (h, w) = img_show.shape
        img_ref = cv2.cvtColor(img2, cv2.COLOR_GRAY2BGR)
                ...
                if y in line:
                    for x in range(w-self.padding):
                        ...
                        if abs(p1 - p2) < self.pixel_value:
                            ...
                            points.append([x, y])
        for point in points:
            cv2.circle(img_show, (point[0], point[1]), 1, (100, 100, 225), -1)
        cv2.imwrite(image_show, img_show)
        #cv2.imwrite(image2, img_ref)
        return len(points)
